 ...
 * @说明:导出降水距平
*/
public class ExportJPToTxtPro {
	public void excute(String tableName,String vTableName,String strFile) throws Exception {
		//1、读取配置
			...
			//5、3-4合并计算距平
			List<CimissMonthData> lsJPResult = null;
			if(tableName.equals("t_month_temp")) {
				lsJPResult = CombinTempData(lsCimissMonthData,lsCimissHosMonthData);
			}
			else {
				lsJPResult = CombinPrecData(lsCimissMonthData,lsCimissHosMonthData);
			}
	 ...
	 * @说明:
	 */
	private static List<CimissMonthData> CombinPrecData(List<CimissMonthData> lsCimissMonthData,List<CimissHosMonthData> lsCimissHosMonthData) {
		...
		for(CimissMonthData cmd:lsCimissMonthData) {
			...
			for(CimissHosMonthData chmd:lsCimissHosMonthData) {
				...
				if(sn.equals(tempSN)) {
					...
					newCMD.setM8(pVal8);
					newCMD.setM9(pVal9);
					newCMD.setM10(pVal10);
					newCMD.setM11(pVal11);
					newCMD.setM12(pVal12);
					lsJPData.add(newCMD);
					break;
				}
			}
	 ...
	 * @说明:
	 */
	private static List<CimissMonthData> CombinTempData(List<CimissMonthData> lsCimissMonthData,List<CimissHosMonthData> lsCimissHosMonthData) {
		List<CimissMonthData> lsJPData = new ArrayList();
		for(CimissMonthData cmd:lsCimissMonthData) {
			String sn = cmd.getStationnum();
			int year = cmd.getYear();
