...
function LSWPageClass() {
    ...
    this.CreateResourceDealPanel = function () {
        ...
        $("#map_div").append(`
            <div id="forcastdata" class="delete panel panel-default" style="display:none;">
                    <div class="paneltitle panel-heading myPanel_Header">
                        <span>预测结果</span><span class="iconfont icon-close close"></span>
                    </div>
    ...
    this.Forcast = function () {
        ...
        var [startDate,endDate] = t.getTimes();
    ...
    this.CreateXLabel = function () {
        var [startDate,endDate] = t.getTimes();
        var labels=[];
        while(!(startDate>endDate)){//不好判断<=
            var strmmdd = startDate.format("MM-dd");
            labels.push(strmmdd);
            startDate.addDays(1);
        }
        var option = t.chartUtil.echart.getOption();
